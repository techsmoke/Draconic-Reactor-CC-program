-- Utility functions for Draconic Reactor CC scripts (techsmoke)
-- Cleaned up: do NOT hard-error if no monitor exists at load time.

-- peripheral identification
local sides = { "left", "right", "back", "front", "bottom", "top" }

local function Identify(name, data) return name == data end

function getPeripheral(id)
  local names = peripheral.getNames()
  for _, v in pairs(names) do
    local Type = peripheral.getType(v)
    if Identify(id, Type) then
      for _, t in pairs(sides) do
        if v == t then return peripheral.wrap(t) end
      end
    end
  end
  return nil
...
function periphSearch(typeName)
  local names = peripheral.getNames()
...
-- monitor related (expects mon = {monitor=<periph>, X=<w>, Y=<h>})
