...
setDefault("lowFieldSoft", 30)          -- start throttling output
...
setDefault("safeTempRestart", 3000)     -- restart allowed below
...
local function alarm(on, msg)
  redstone.setOutput(S("alarmSide"), on)
  if on and msg then logEvent("ALARM: "..msg) end
  if on and S("alarmSpeakerEnabled") and #speakers > 0 then
    for _, sp in ipairs(speakers) do
      pcall(function()
        if sp.playSound then sp.playSound("minecraft:block.note_block.pling", 1.0, 1.0) end
        if sp.speak then sp.speak(msg or "Reactor alarm") end
      end)
    end
  end
end
...
local function setupButtons()
  ...
  local function changeOut(delta)
    local v = gateGet(gateOut) + delta
    v = clamp(v, 0, 500000000)
    gateSet(gateOut, v)
    setAction("Output -> "..f.format_int(v).." rf/t", colors.lightBlue)
    logEvent("setOutput "..v)
  end

  local bx = 4
  local by = 32
  button.setButton("out-1m", "<<", function() changeOut(-1000000) end, bx, by, bx+3, by+2, 0, 0, colors.blue); bx=bx+5
  button.setButton("out-100k", "<", function() changeOut(-100000) end, bx, by, bx+3, by+2, 0, 0, colors.blue); bx=bx+5
  button.setButton("out+100k", ">", function() changeOut(100000) end, bx, by, bx+3, by+2, 0, 0, colors.blue); bx=bx+5
  button.setButton("out+1m", ">>", function() changeOut(1000000) end, bx, by, bx+3, by+2, 0, 0, colors.blue)
...
local outTarget = gateGet(gateOut)
...
local function throttleOutput(ri)
  -- reduce output if unsafe (shield/heat/saturation can't keep up)
  local maxTemp = S("maxTemp")
  local lowFieldHard = S("lowFieldHard")
  local lowFieldSoft = S("lowFieldSoft")

  local satPct = pct(ri.energySaturation, ri.maxEnergySaturation)
  local fieldPct = pct(ri.fieldStrength, ri.maxFieldStrength)
  local outNow = gateGet(gateOut)

  -- hard emergencies
  if ri.temperature >= maxTemp or (fieldPct <= lowFieldHard and ri.status == "running") then
    gateSet(gateOut, 0)
    alarm(true, "Reactor emergency: output cut")
    setAction("EMERGENCY: output cut", colors.red)
    return
  end

  -- soft protection
  local outMax, tempSoft, rampStep, _ = params()
  local want = clamp(outTarget, 0, outMax)

  -- if field/sat/temp are low/high, scale down want
  if fieldPct < lowFieldSoft then
    want = math.min(want, math.floor(outNow * 0.75))
    setAction("Throttle: low field", colors.orange)
  end
  if satPct < 25 then
    want = math.min(want, math.floor(outNow * 0.80))
    setAction("Throttle: low sat", colors.orange)
  end
  if ri.temperature >= tempSoft then
    want = math.min(want, math.floor(outNow * 0.70))
    setAction("Throttle: hot", colors.orange)
  end

  want = clamp(want, 0, outMax)

  if want < outNow then
    gateSet(gateOut, want)
    outTarget = want
  end

  if ri.temperature < tempSoft and fieldPct > lowFieldSoft and satPct > 30 then
    alarm(false)
  end
end

local function controlInput(ri)
  -- Aim field strength near targetStrength by adjusting input gate (injector power)
  local target = S("targetStrength")
  local lowHard = S("lowFieldHard")

  local fieldPct = pct(ri.fieldStrength, ri.maxFieldStrength)
  local err = target - fieldPct

  local desired = curIn
  -- If field is too low, push input up strongly
  if fieldPct <= lowHard + 2 then
    desired = math.max(desired, curIn + 500000)
    setAction("Boost field (hard)", colors.red)
  else
    desired = curIn + (err * 15000) -- coarse proportional
  end

  desired = clamp(desired, S("inMin"), S("inMax"))

  local a = clamp(S("inputSmooth"), 0, 1)
  curIn = math.floor((curIn * (1 - a)) + (desired * a) + 0.5)
  curIn = clamp(curIn, S("inMin"), S("inMax"))

  gateSet(gateIn, curIn)
end

local lastRamp = 0

local function rampOutput()
  local outMax, _, rampStep, rampInterval = params()
  if os.clock() - lastRamp < rampInterval then return end
  lastRamp = os.clock()

  -- ramp up to outTarget but clamp to outMax
  local outNow = gateGet(gateOut)
  local want = clamp(outTarget, 0, outMax)

  if outNow < want then
    gateSet(gateOut, math.min(outNow + rampStep, want))
  elseif outNow > want then
    gateSet(gateOut, math.max(outNow - rampStep, want))
  end
end
